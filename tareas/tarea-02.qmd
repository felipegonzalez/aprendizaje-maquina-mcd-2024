...
En este ejemplo repasamos las descomposiciones del error que vimos en la clase
anterior. En lugar de comparar k-vecinos vs regresión lineal, compararemos
regresión polinomial a lo largo de distintos grados.
...
p_val <- 30 / 10000
split_ent_pr <- initial_validation_split(
  muestra,
  prop =c(30/10000, 9000/10000))
muestra_entrena <- training(split_ent_pr)
```
...
ajustes <- tune_grid(flujo_polinomio,
                     resamples = validation_set(split_ent_pr),
