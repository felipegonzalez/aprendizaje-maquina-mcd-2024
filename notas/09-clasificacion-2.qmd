...
vertical ponemos el porcentaje de  verdaderos positivos encontrados del total
de positivos (sensibilidad o recall acumulado).
...
ggplot(gain_tbl, 
  ...
  geom_abline(colour = "gray") + 
  coord_equal() +
  xlab("% Clasificado como positivo") + 
  ylab("% Positivos encontrados")
```

La recta gris representa un clasificador al azar (si clasificamos escogemos al azar
x% como positivos, esperamos encontrar x% de todos los positivos).


Otra posibilidad es graficar en el
eje vertical la sensibilidad entre el % de clasificados como positivos (o % probado):
...
en encontrar positivos que una estrategia de tomar casos al azar. Por las mismas
